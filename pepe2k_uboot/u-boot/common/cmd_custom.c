 ...
 * Do factory reset 
 */
#if defined(CONFIG_CMD_FIRSTBOOT)
int do_firstboot(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	
	uint addr = (uint)0x9f04004c;  // pradzioje esancio md5 adresas
	char linebuf[16];
	...
	for (ii = 0; ii < 239; ii += 1) {
		
		cp = (u_char *)linebuf;	
		uint addrr = 0x9f10fff0 + (0x10000 * ii);
		cpp = (u_char *)addrr; 
		state = 0;
		...
		if(state == 0){
			addrr += 0x10;
			puts("Erasing FLASH...\n");
			if(flash_sect_erase((ulong)addrr, (addrr + 0xffff))){
				return 1;	
			}

			char *dead = (char *)0xDEADC0DE;
			rc = flash_write((char *)&dead, addrr, sizeof(dead));
...
U_BOOT_CMD(firstboot, 1, 1, do_firstboot, "Do factory reset (This will erase all settings)\n", NULL);
#endif /* CONFIG_CMD_LED */
