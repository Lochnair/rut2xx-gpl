...
#define STATE_SERIAL_REQUEST	3

static uint8_t serial_request_magic[] = {
	0x52, 0x45, 0x51, 0x55, 0x45, 0x53, 0x54, 0x20, 0x53, 0x45, 0x52, 0x49, 0x41, 0x4c
};
...
void httpd_appcall(void){
	...
	char kernel_md5[17] = "\0";
	char fw_md5[17] = "\0";
	char crc_string[11] = "\0";
	char router_name[50] = "\0";
	char current_name[13] = "RUT2xx      ";
	char *ptr;
	unsigned int md5_ok=1;
		...
		case HTONS(80):
			...
			if(uip_newdata() && hs->state == STATE_NONE){
				...
				if (uip_appdata[0] == ISO_G && uip_appdata[1] == ISO_E && uip_appdata[2] == ISO_T && (uip_appdata[3] == ISO_space || uip_appdata[3] == ISO_tab)){
					...
					if (!memcmp(uip_appdata, serial_request_magic, len)) {
						void *addr = KSEG1ADDR(CFG_FLASH_BASE) + OFFSET_MNF_INFO + OFFSET_SERIAL_NUMBER;
				...
				if(hs->state == STATE_FILE_REQUEST){ // get file or firmware upload?
				...
				if(hs->state == STATE_UPLOAD_REQUEST){
					...
					if(hs->upload >= hs->upload_total){
						if(webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_UBOOT){
							...
							router_name[0] = 0;						//11-crc_code;10 - uboot_version; 12 - router_name;
							strncpy(router_name, (const char *)(ptr + hs->upload_total-10-12), 12);
							//printf("\nrouter_name: [%s]==[%s]\n\n", router_name, current_name);
							...
							crc = atoi(crc_string);
							crc_current = crc32(0, (const unsigned char *)CONFIG_LOADADDR, hs->upload_total);
							...
							ptr = (char *)CONFIG_LOADADDR;
							router_name[0] = 0;
							strncpy(router_name, (const char *)(ptr + 28), 6);
							router_name[7] = 0;

							kernel_md5[0] = 0;
							strncpy(kernel_md5, (const char *)(ptr + 76), 16);
							kernel_md5[17] = 0;
							fw_md5[0] = 0;
							strncpy(fw_md5, (const char *)(ptr + hs->upload_total-42-12), 16);
							fw_md5[17] = 0;

							for(i =0; i <17; i++){
								if( kernel_md5[i] != fw_md5[i] ){
									md5_ok=0;
									break;
								}
							}
						...
						if(!webfailsafe_upload_failed){
							...
							} else if ((webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_UBOOT) && ( strncmp(router_name, current_name, 12) != 0 )) {
								webfailsafe_upload_failed = 1;
								fs_open(file_fail_name_html.name, &fsfile);
							} else if ((webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_FIRMWARE) && ( md5_ok != 1 )) {
								webfailsafe_upload_failed = 1;
								fs_open(file_fail_checksum_html.name, &fsfile);
							} else if ((webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_FIRMWARE) && ( strncmp(router_name, current_name, 6) != 0 )) {
								webfailsafe_upload_failed = 1;
								fs_open(file_fail_name_html.name, &fsfile);
							} else {
