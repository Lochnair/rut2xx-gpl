...
ART_FILE="scripts/art_tlt_rut200.bin"
BOOTROM_FILE="scripts/u-boot_mod_tlt_rut200.bin"
CONFIG_FILE="scripts/config_tlt_rut200.bin"
WEBUI_FILE=""
...
if [ -s "$ART_FILE" ] && [ -s "$BOOTROM_FILE" ] && [ -s "$CONFIG_FILE" ] && [ -s "$WEBUI_FILE" ]; then
	...
	version="${master_name%_*}"
	master_name="${master_name%_*}_MASTER_STENDUI.bin"
	# create the master image
	cat master_nopad.img pad.img > $master_name
	rm -f master_nopad.img pad.img
	# fw versijos irasymas i master fw gala.
	echo -ne "$version" | dd conv=notrunc obs=16 seek=1048570 of=$master_name
else
