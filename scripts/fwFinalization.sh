...
GENERATED_FW_FILE="openwrt-ar71xx-generic-tlt-rut200-squashfs-sysupgrade.bin"
# temporary md5 file
MD5_FILE="image_md5.tmp"
SEADRIVE_PATH_1="/Firmwares/0_Testavimui/RUT2XX/"
...
if [ "$fw_name" != "image" ]; then
	...
	#add fw model
	printf '%.4s' "$fw_model" >> "$GENERATED_FW_FILE"
	if [ "$?" != 0 ];
	then
		echo "Adding fw model to WebuiFW image $GENERATED_FW_FILE failed!"
		exit 1
	fi 


	#add from hardware revision
	printf '%.4s' "$hw_rev_from" >> "$GENERATED_FW_FILE"
	if [ "$?" != 0 ];
	then
		echo "Adding from hardware revision to WebuiFW image $GENERATED_FW_FILE failed!"
		exit 1
	fi

	#add to hardware revision
	printf '%.4s' "$hw_rev_to" >> "$GENERATED_FW_FILE"
	if [ "$?" != 0 ];
	then
		echo "Adding to hardware revision to WebuiFW image $GENERATED_FW_FILE failed!"
		exit 1
	fi
...
if [ -f $BOOTROM_FILE -a -f "$ART_FILE" -a -f "$CONFIG_FILE" ]; then
	...
	echo -ne "$version" | dd conv=notrunc obs=16 seek=1048570 of=master_stendui.img
	...
	if [ "$fw_name" != "image" ]; then
		...
		if [ -f tltFws/${version}_WEBUI.bin ]; then
			...
			for ii in {0..999}; do
				...
				if [ ! -f $testfile ]; then
					...
					cp master_stendui.img tltFws/${version}_${int3l}_MASTER_STENDUI.bin
			...
			cp master_stendui.img tltFws/${version}_MASTER_STENDUI.bin
		fi
		...
		if [ $upload_fw -eq 1 ] && [ ! -e "${upload_dir}" ]; then
			...
			case "$branch_name" in
				master*)
					cp master_stendui.img "${upload_dir}/${version}_MASTER_STENDUI.bin"
		...
		cp master_stendui.img ${GENERATED_FW_FILE%_WEBUI.bin*}_MASTER_STENDUI.bin
	fi
