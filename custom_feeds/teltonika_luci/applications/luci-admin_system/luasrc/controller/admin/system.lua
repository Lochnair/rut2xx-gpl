...
function index()
	...
	entry({"admin", "system", "admin", "monitoring"}, cbi("admin_system/admin_access_control_remote"), _("RMS"), 8).leaf = true
	entry({"admin", "system", "admin", "root_ca"}, template("admin_system/root_ca"), _("Root CA"), 9).leaf = true
	...
	if(wimax_file) then
		entry({"admin", "system", "admin", "wimax"}, cbi("admin_system/wimax"), _("WiMAX"), 10).leaf = true
	end
...
function render_backup_and_throw_error(redirect)
	...
	luci.template.render("admin_system/backup", {
		download_error 	= 1,
		error_message 	= error_message .. " (" .. "Error: " .. auto_update_conf_err .. ")"
	})
	return
end
...
function check_backup()
    ...
    if fw_version_in_new_config_len < 12 or this_device_fw_version < fw_version_in_new_config then
            write_new_config_ok = 0
    end
