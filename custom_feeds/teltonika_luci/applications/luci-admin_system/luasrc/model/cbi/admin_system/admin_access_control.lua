...
-- 	translate("Changes the administration password, log level and provides SSH access control."))
	...
	function o.write(self, section)
		...
		if fval then
		        m3.uci:set("uhttpd", "main", "_sshWanAccess", fval)
		else
		        m3.uci:set("uhttpd", "main", "_sshWanAccess", "0")
		end
...
if fs.access("/etc/config/uhttpd") then
	...
	dummy.customValue = translate("Redirect to HTTPS enabled!")
	dummy.customHint = translate("Enable remote HTTPS access for remote WEB access to work correctly")
	dummy:depends({redirect_https="1", _httpWanAccess="1"})
...
function m3.on_after_commit(self)
    ...
    local ssh_enabled = sys.exec("uci get uhttpd.main._sshWanAccess")
    if ssh_enabled then
        ssh_enabled = ut.trim(ssh_enabled)
    else
        ssh_enabled = "0"
    end
    ...
    local ssh_port = sys.exec("uci get dropbear.@dropbear[0].Port")
    if ssh_port then
        ssh_port = ut.trim(ssh_port)
    else
        ssh_port = "22"
    end
	...
	m3.uci:foreach("firewall", "redirect", function(s)
		if s["name"] == "tlt_allow_remote_http_through_DMZ" or s["name"] == "tlt_allow_remote_https_through_DMZ" or s["name"] == "tlt_allow_remote_ssh_through_DMZ" then
    ...
    if dmz_enabled == "1" then
        if ssh_enabled == "1" then
            ...
            sys.exec("uci set firewall." ..config_value.. ".src_dport='" ..ssh_port.. "'")
            sys.exec("uci set firewall." ..config_value.. ".dest_ip='".. lan_ip .."'")
            sys.exec("uci set firewall." ..config_value.. ".dest_port='"..ssh_port.."'")
            sys.exec("uci set firewall." ..config_value.. ".name='tlt_allow_remote_ssh_through_DMZ'")
