...
function index()
	...
	entry({"admin", "status", "auto_select"}, call("connect_network_auto"), nil, nil)
...
function connect_network_auto()
	...
	luci.sys.exec("/etc/init.d/operctl restart")
...
function connect_network_manual(number)
	luci.http.prepare_content("application/json")

	uci:delete("simcard", "sim1", "mode")
	uci:set("simcard", "sim1", "numeric", number)
	uci:commit("simcard")
...
function connect_network_manual_auto(number)
	luci.http.prepare_content("application/json")

	uci:set("simcard", "sim1", "mode", "Manual-Auto")
	uci:set("simcard", "sim1", "numeric", number)
	uci:commit("simcard")
...
function reconnect_interval()
	...
	uci:set("simcard", "sim1", "reconnect", interval)
	uci:commit("simcard")
end
...
function connect_rsp()
	...
	if process_running("operctl") then
...
function connect_network_switch()
	local number = luci.http.formvalue("numeric")
	local mode = luci.http.formvalue("mode")
	...
	if mode == "Manual-Auto" then
		connect_network_manual_auto(number)
	else
		connect_network_manual(number)
	end
end
