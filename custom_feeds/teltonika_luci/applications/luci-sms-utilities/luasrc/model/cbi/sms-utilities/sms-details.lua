...
src = s:option(Value, "smstext", translate("SMS text"), translate("SMS text that is required to trigger this rule. Allowable characters (a-zA-Z0-9!@#$%&*+-/=?^_`{|}~.). If authorization is used, password or serial number should be in the beginning of the message and separated with a space character from the rest of the configured message text"))
src.datatype = "fieldvalidation('^[a-zA-Z0-9!@#$%%&*+/=?^_`{|}~. -]+$',0)"

dummy = s:option(DummyValue, "getinfo_more", translate(""))
dummy.default = translate("SMS text, which lets you see the next part of the message")
...
dummy.default = translate("SMS text, which let you to change your mobile settings to SIM.<br>E.g. \"cellular apn=internet.gprs dialnumber=*99***1# auth_mode=pap service=3gonly username=user password=user\"<br><br>")
dummy.rawhtml = true
...
dummy.default = translate("SMS text, which let you to change router profile. After secret word you have to write profile name.<br> E.g. \"pr profile_name\"<br><br>")
dummy.rawhtml = true
dummy:depends("action", "change_profile")

dummy = s:option(DummyValue, "getinfo_vpn", translate(""))
dummy.default = translate("SMS text, which let you to stop or start OpenVPN instance. After secret word you have to write OpenVPN name. E.g. \"vpnon client_name\" or \"vpnoff client_name\"<br><br>")
dummy.rawhtml = true
...
dummy.default = translate("SMS text, which let you to upgrade your router form server. After this rule execute, router will reboot.<br> E.g. \"fw_upgrade\"<br><br>")
dummy.rawhtml = true
dummy:depends("action", "fw_upgrade")

dummy = s:option(DummyValue, "getinfo_config_update", translate(""))
dummy.default = translate("SMS text, which let you to update your router config form server. After this rule execute, router will reboot.<br> E.g. \"config_update\"<br><br>")
dummy.rawhtml = true
...
-- gsmctl = s:option(Value, "mac", translate("Mac address"), translate("Set mac address to be woken up after an SMS is sent"))
-- gsmctl:depends("action", "test")
...
outputtype:value("sim1", translate("SIM"))
