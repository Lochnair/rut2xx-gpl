...
function fix_user_configuration(required_interfaces)
	uci:foreach("qos", "interface", function(s)
		...
		-- New configuration does not define a 'wan' interface
		if current_interface == "wan" then
			uci:delete("qos", current_interface)
		end
		for key, value in pairs(required_interfaces) do
			...
			elseif current_interface == "ppp" and key == "Mobile" then
