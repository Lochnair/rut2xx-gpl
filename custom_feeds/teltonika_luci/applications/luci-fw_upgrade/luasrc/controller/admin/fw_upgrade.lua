...
function index()
	...
	entry({"admin", "system", "flashops","check_download"}, call("check_download_state"), nil, nil)

end

function check_download_state()
	local auto_update_err_code = luci.sys.call("/usr/sbin/auto_update.sh check")
	local rv = {
		error_code = auto_update_err_code
	}
...
function is_connection_available()
	local download_error = luci.sys.call("/usr/sbin/auto_update.sh check")
	if not (download_error == 0) then
...
function render_flashops_and_throw_error(redirect)
	...
	if download_error == 8 and (redirect == 1) then
		error_message = error_message .. " Connection dropped."
	end

	if download_error == 8 and (redirect == 0) then
...
function check_for_update()
	local auto_update_err_code 	= luci.sys.call("/usr/sbin/auto_update.sh forced_check")
	local remote_fw_version 	= luci.sys.exec("cat /tmp/version_remote")
	luci.sys.exec("rm -f /tmp/version_remote")
	local err_code = 0
	if remote_fw_version == nil or remote_fw_version == '' then
		if not auto_update_err_code == 0 then
			if auto_update_err_code == 8 then
				err_code = 1
...
function download_fw()
	...
	if (redirect == 1) or not (is_connection_available() == 0) then
