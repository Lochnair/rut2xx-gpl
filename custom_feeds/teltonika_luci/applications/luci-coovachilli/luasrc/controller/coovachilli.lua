...
x:foreach("wireless", "wifi-iface", function(s)
	...
	if s.hotspotid then
		if x:get("coovachilli", s.hotspotid, "enabled") == "1" then
...
entry( { "admin", "services", "hotspot" }, alias("admin", "services", "hotspot", "general"), _("Hotspot"), 90)
	...
	if listofssids[1] ~= nil then
		entry({"admin", "services", "hotspot", "statistics"}, alias("admin", "services", "hotspot", "statistics", listofssids[1]), _("Statistics"), 7)
	...
	if #listofids > 0 then
		entry({"admin", "services", "hotspot", "hotspot_scheduler"}, alias("admin", "services", "hotspot", "hotspot_scheduler", listofids[1]["id"]), _("Restricted Internet Access"), 2)

		for Index, Value in pairs( listofids ) do
			entry({"admin", "services", "hotspot", "hotspot_scheduler", Value.id}, cbi("hotspot_scheduler"), _(Value.ssid), Index).leaf = true
		end
	else
	...
	entry({"admin", "services", "hotspot", "loging"}, alias("admin", "services", "hotspot", "loging", "configuration"), _("Logging"), 3)
		...
		entry({"admin", "services", "hotspot", "loging", "smsotplog"}, template("chilli/smsotplog"), _("SMS OTP Log"), 3).leaf = true
...
function enable_disable()
	...
	if section then
		...
		if http_enb then
			uci:set("uhttpd", "hotspot", "disabled", "0")
		else
