...
local gps = utl.trim(sys.exec("uci get -q hwinfo.hwinfo.gps")) or "0"
local in_out = utl.trim(sys.exec("uci get -q hwinfo.hwinfo.in_out")) or "0"
local rs485 = utl.trim(sys.exec("uci get -q hwinfo.hwinfo.rs485")) or "0"
local rs232 = utl.trim(sys.exec("uci get -q hwinfo.hwinfo.rs232")) or "0"
...
--o:value("FW upgrade", translate("FW upgrade"))
if show then
...
if show then
	o:value("sms", translate("SMS Utilities"))
	o:value("mobile traffic", translate("Mobile traffic"))
	o:value("multivan", translate("Multiwan"))
...
o:value("gre tunnel", translate("GRE tunnel"))
...
o:value("content blocer", translate("Content blocker"))
...
o:depends("event", "FW upgrade")
o.default = m.uci:get("events_reporting", arg[1], "eventMark")
function o.write(self, section, value)
	local ev = luci.http.formvalue("cbid.events_reporting."..arg[1]..".event")
	if ev == "FW upgrade" then
		m.uci:set("events_reporting", section, "eventMark", value)
		m.uci:save("events_reporting")
		m.uci:commit("events_reporting")
	end
...
o:value("from", translate("SMS received"))
o:depends("event", "SMS")
o.default = m.uci:get("events_reporting", arg[1], "eventMark")
function o.write(self, section, value)
	local ev = luci.http.formvalue("cbid.events_reporting."..arg[1]..".event")
...
o:value("all", translate("All"))
o:value("was succesful", translate("Successful authentication"))
o:value("not succesful", translate("Unsuccessful authentication"))
...
-- o:value("all", translate("All"))
-- o:value("SIM 1 to SIM 2", translate("SIM 1 to SIM 2"))
-- o:value("SIM 2 to SIM 1", translate("SIM 2 to SIM 1"))
...
o:value("all", translate("All"))
o:value("Signal strength droped below -113 dBm", translate("-121dBm -113dBm"))
o:value("Signal strength droped below -98 dBm", translate("-113dBm -98dBm"))
o:value("Signal strength droped below -93 dBm", translate("-98dBm -93dBm"))
o:value("Signal strength droped below -75 dBm", translate("-93dBm -75dBm"))
o:value("Signal strength droped below -60 dBm", translate("-75dBm -60dBm"))
o:value("Signal strength droped below -50 dBm", translate("-60dBm -50dBm"))
...
rstr_timer:value("45", translate("45 min"))
rstr_timer.default = "10"
...
function m.on_parse(self)
	if m:formvalue("cbid.events_reporting."..arg[1].."._testMail") then
		...
		if secure == "1" then
			luci.sys.call("(echo -e \"subject:".. subject .."\nfrom:".. senderEmail .."\n".. message .."\" | sendmail -v -H \"exec openssl s_client -quiet -connect ".. smtpIP ..":".. smtpPort .." -tls1 -starttls smtp\" -f ".. senderEmail .." -au".. username .." -ap".. passwd .." ".. allRecMail .."; echo $? >/tmp/sendmail ) &")
			for time = 0, 30, 1 do
		...
		elseif secure == "0" then
			luci.sys.exec("(echo -e \"subject:".. subject .."\nfrom:".. senderEmail .."\n".. message .."\" | sendmail -S \"".. smtpIP ..":".. smtpPort .."\" -f ".. senderEmail .." -au".. username .." -ap".. passwd .." ".. allRecMail .."; echo $? >/tmp/sendmail )&")
			for time = 0, 30, 1 do
				status = luci.util.trim(luci.sys.exec("cat /tmp/sendmail"))
				if status ~= "" then
					break
