...
k = s:option(Value, "retry", translate("Retry count"), translate("Number of failed to receive ICMP (Internet Control Message Protocol) echo reply packets. Range [0 - 9999]"))
k.default = "2"
--k:depends("reboot", "1")
...
function msg.cfgvalue(self, section)
	...
	if time_value and retry_value then
		local time = time_value + (time_value * retry_value)
