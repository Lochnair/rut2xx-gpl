...
v = s:option(DummyValue, "action", translate("Action"), translate("Action after the defined number of unsuccessfull retries (no echo reply for sent ICMP packet received)"))
	function v.cfgvalue(self, section)
		local value = self.map:get(section, self.option)
		local actions = {"Reboot", "Modem restart", "Restart mobile connection", "(Re)register", "None"}
