...
wpakey = s:option(Value, "_wpa_key", translate("Key"), translate("Specifies encrypted string"))
...
wepslot = s:option(ListValue, "_wep_key", translate("Used key slot"), translate("Specifies which encrypted string will be used"))
...
local slot
for slot=1,4 do
	wepkey = s:option(Value, "key" .. slot, translatef("Key #%d", slot), translate("Specifies encrypted string"))
	wepkey:depends("encryption", "wep-open")
	wepkey:depends("encryption", "wep-shared")
	wepkey.datatype = "wepkey"
	wepkey.rmempty = true
	wepkey.noautocomplete = true
