...
time = section:option(Value, "time", translate("Lease time"), translate("Expire time for leased addreses. Minimum value is 2 minutes."))
	...
	function time.write(self, section, value)
		...
		uci:set("dhcp", section, "leasetime", value .. "" .. letter)
...
function validate_leasetime()
	...
	if not letter or #letter == 0 or not leasetime or #leasetime == 0
		or (letter == "h" and tonumber(leasetime) <= 0)
		or (letter == "m" and tonumber(leasetime) < 2) then
...
function validate_ip_range()
	...
	if not start or #start == 0 or tonumber(start) == 0 then
		dhcp.message = "err: Invalid start."
		return false
	elseif not limit or #limit == 0 or tonumber(limit) == 0 then
		dhcp.message = "err: Invalid limit."
		return false
	elseif not max_range or (tonumber(limit) + tonumber(start)) > (max_range - 1) then
