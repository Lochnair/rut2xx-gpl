...
-- 	ifconf_remote.rmempty = false
	...
	function route_mask:validate(Value)
		...
		if txtip == networkip then
			lan_t = {}
			net_t = {}
			string.gsub(lan_ip, "%d+", function(i) table.insert(lan_t, i) end)
			string.gsub(networkip, "%d+", function(i) table.insert(net_t, i) end)
			string.gsub(lan_ip, "%.(%d+)$", function(i) lan_ip = tonumber(i) end)
			string.gsub(networkip, "%.(%d+)$", function(i) networkip = tonumber(i) end)
			string.gsub(broadcast, "%.(%d+)$", function(i) broadcast = tonumber(i) end)
			if lan_t[1] == net_t[1] and lan_t[2] == net_t[2] and lan_t[3] == net_t[3] and
				lan_ip > networkip and lan_ip < broadcast then
...
-- 	end
	...
	function o:parse(section)
		...
		if auth_val=="tls" or auth_val=="tls/pass" then
...
if TMODE == "client" then
	...
	auth_key_direction = s:option( ListValue, "auth_key_direction", translate("HMAC key direction"), translate("HMAC authentication key direction value is arbitrary and must be opposite between communicating parties (or omitted entirely)"))
		auth_key_direction:depends({_tls_auth="1"})
	...
	dec_key = s:option( Value, "decrypt", translate("Private key decryption password (optional)"), translate("Decrypt private key with password.(Optional)"))
		dec_key.noautocomplete = true
		dec_key.password = true
		dec_key:depends({_auth="tls"})
		dec_key:depends({_auth="tls/pass"})
...
function m.on_commit(map)
	...
	if TMODE == "server" then
		...
		if (auth ~= "skey" or dev ~= "tun") then
			local keepalive = m:formvalue("cbid.openvpn." .. VPN_INST .. "._keepalive_tls")
