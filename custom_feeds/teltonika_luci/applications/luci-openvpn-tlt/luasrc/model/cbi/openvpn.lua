...
s.addremove = true
s.anonymous = true
s.template = "openvpn/tblsection_openvpn"
...
function name.cfgvalue(self, section)
	if (section) then
		local name_is_hexed = util.trim(sys.exec("uci -q get openvpn." .. section .. ".name_is_hexed")) or "0"
		if name_is_hexed == "1" then
			return hex_to_string(section)
		else
			return section
		end
	else
...
-- 		end
	...
	if delButtonPress then
		if string.sub(configName,1,string.len("server_"))== "server_" then
			uci:foreach("openvpn", "client",function(s)
				if s.sname == configName then
					uci.delete("openvpn",s[".name"])
				end
			 end)
		end
...
function openvpn_new(self,name, exist)
	...
	local num_vpn_clients = util.trim(sys.exec("cat /etc/config/openvpn |grep 'config openvpn' |grep 'client_' -c")) or "0"
	local num_vpn_servers = util.trim(sys.exec("cat /etc/config/openvpn |grep 'config openvpn' |grep 'server_' -c")) or "0"

	if role == "client" and tonumber(num_vpn_clients) > 4 then
		m.message = translatef("err: Maximum OpenVPN client count has been reached")
		return
	end
	if role == "server" and tonumber(num_vpn_servers) > 0 then
		m.message = translatef("err: Only one OpenVPN server instance is allowed")
		return
	end
	...
	elseif name:match("%W") then
		m.message = translatef("err: Name %s contains special characters.", name)
	...
	elseif name and #name > 0 and role then
		...
		--else
			...
			name = (role.."_"..name)
					...
					--hex the instance name (because some symbols are forbiden in uci config name)
