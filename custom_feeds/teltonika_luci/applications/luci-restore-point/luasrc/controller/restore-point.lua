...
function apply()
	...
	elseif step == 3 then
		...
		eventlog:insert(t)
		filelink = luci.http.formvalue("filelink")
		fork_exec("/etc/init.d/uhttpd stop; sleep 1; /sbin/restore-point -r "..filelink.."; if [ $? -ne 0 ]; then /etc/init.d/dropbear start; /etc/init.d/uhttpd start; else reboot -t; fi")
	end
end
