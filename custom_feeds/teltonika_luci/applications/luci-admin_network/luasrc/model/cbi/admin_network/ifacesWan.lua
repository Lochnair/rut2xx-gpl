...
function use_wan_as_lan_enabled()
	local a = uci:get("network", "lan", "ifname")
	if a:match("eth1") then
		return true
	else
		return false
	end
...
wan_sec = m:section(TypedSection, "interface", translate("Operation Mode"))
	...
	function wan_sec.parse(self, novld)
		local uci = require("uci").cursor()
		local access_points = 0
		uci:foreach("wireless", "wifi-iface", function(s) if (s["mode"] or "") == "ap" then access_points = access_points + 1 end end)
		if access_points > 1 then
			self.wifi_disabled = 1
			m.message = "wrn: Wireless Station is not available with more than one Access Point."
		end
...
main_wan = wan_sec:option(Flag, "main", translate("Main WAN"))
	...
	function main_wan.cfgvalue(self, section)
		local ifname = self.map:get(section, "ifname") or ""
		if ifname == "eth1" and use_wan_as_lan_enabled() == true then
...
back_wan = wan_sec:option(Flag, "enabled", translate("Mode"))
	...
	function back_wan.write(self, section, value)
		...
		local ppp_ifname = self.map:get("ppp", "ifname") or ""
		local ifname = self.map:get(section, "ifname") or ""

		if (value and value == "1") and not main_wan then
			if ifname == ppp_ifname then
					self.map:set("ppp", "metric", "10")
			end
...
ip_v4 = wan_sec:option(DummyValue, "ipaddr", translate("IP Address"))
	...
	function ip_v4.cfgvalue(self, section)
		local value
		local net = nw:get_network(section)
		local device = net and net:get_interface()
		value = self.map:get(section, self.option)
		if not value then
			if device then
				for _, a in ipairs(device:ipaddrs()) do
					if device:name() == "usb0" then
						value = nw:wimaxCGICall({ call ="extip" })
					else
						value = a:host():string()
					end
				end
...
function m.on_parse(self)
	if luci.http.formvalue("cbi.apply") then
		...
		on_off_balancig(mode)
	end
end

function on_off_balancig(mode)
