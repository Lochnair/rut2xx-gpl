...
function encryption(enc)
	...
	elseif enc == "wep-open" then
		ent = i18n.translate("WEP open system")
	elseif enc == "wep-shared" then
		ent = i18n.translate("WEP shared key")
	elseif enc == "psk" then
...
function iwinfo_parser()
	...
	for line in str:gmatch("[^\r\n]+") do
		if line:find("ESSID: ") then
			i=i+1
			nets[i]= { }
			nets[i]["got_name"] = false
			a,b = line:find("ESSID: ")

			if line:sub(b,#line) == " unknown" then
				nets[i]["name"]="N/A"
				nets[i]["got_name"] = false
			else
				nets[i]["name"]=line:sub(b+2,#line-1)
				nets[i]["got_name"] = true
			end
...
function wifinet.active_mode(self)
	local m = _stror(self.iwinfo.mode, self.iwdata.mode) or _uci_state:get("wireless", self.sid, "mode") or "ap"
...
function get_openvpn()
	...
	for openvpn_fullname in openvpns:gmatch("[^\r\n]+") do
		...
		local status_file_path = "/tmp/openvpn-" .. openvpn_fullname .. ".status"
		local openvpn_status_log = nfs.readfile(status_file_path, 65536)
		local is_connected = false

		-- If the server is not running the status file will share the same format as the clients, therefore
		-- we can simply check whether the line TCP/UDP read bytes, 0 exists to determine it's connection state.
		if openvpn_status_log and not string.match(openvpn_status_log, "TCP/UDP read bytes,0") and #openvpn_status_log > 0 then
			is_connected = true
		end
		...
		if openvpns_dev == "tap" then
			...
			started 		= utl.trim(sys.exec("cat /tmp/state/ifuptime.log 2>/dev/null | grep -w tap0 | awk -F'=' '{print $2}' "))
		...
		elseif is_connected then
			...
			if openvpn_type == "server" then
				openvpns_status 	= "Running"
			else
				openvpns_status 	= "Connected"
			end

			openvpns_ip 		= utl.trim(sys.exec("ifconfig "..openvpns_dev.." 2>/dev/null | grep 'inet addr:' | awk -F':' '{print $2}' | awk -F' ' '{print $1}'"))
			openvpns_mask 	= utl.trim(sys.exec("ifconfig "..openvpns_dev.." 2>/dev/null | grep 'Mask:' | awk -F':' '{print $4}'"))
			started 		= utl.trim(sys.exec("cat /tmp/state/ifuptime.log 2>/dev/null | grep -w "..openvpns_dev.." | awk -F'=' '{print $2}' "))
...
function get_services()
	...
	returnTable = {}
	---------------------VRRP[0]---------------------------------
	local vid1_enabled = utl.trim(sys.exec("uci -q get vrrpd.vid1.enabled")) or "0"
	...
	local openvpn_server = utl.trim(sys.exec("uci show openvpn | grep openvpn.7365727665725F | grep -w \"enable='1'\" | wc -l")) or "0"
	local openvpn_client = utl.trim(sys.exec("uci show openvpn | grep openvpn.636C69656E745F | grep -w \"enable='1'\" | wc -l")) or "0"
	...
	table.insert(returnTable, tablenew)
	---------------------GPS[6]--------------------------------
	local gps_service = utl.trim(sys.exec("uci -q get gps.gps.enabled")) or "0"
	tablenew = { gps_service}
	table.insert(returnTable, tablenew)
	---------------------Hotspot[7]---------------------------------
	local hotspot = utl.trim(sys.exec("uci -q get coovachilli.hotspot1.enabled")) or "0"
	local ftp = utl.trim(sys.exec("uci -q get coovachilli.ftp.enabled")) or "0"
	...
	table.insert(returnTable, tablenew)
	---------------------GRE Tunnel[9]---------------------------------
	local gre_enable = utl.trim(sys.exec("uci show gre_tunnel | grep -w \"enabled='1'\" | wc -l")) or "0"
	tablenew = { gre_enable }
	table.insert(returnTable, tablenew)
	---------------------Input/output[10]---------------------------------
	...
	table.insert(returnTable, tablenew)
	---------------------SMS_utils[11]---------------------------------
	local sms = utl.trim(sys.exec("uci show sms_utils | grep rule | grep -w \"enabled='1'\" | wc -l")) or "0"
	tablenew = { sms }
	table.insert(returnTable, tablenew)
	---------------------Firewall[12]--------------------------------
...
function new_get_info()
	...
	if vid and pid then
		moduleVidPid = string.format("%s:%s", vid, pid)
	end
	...
	local options = {
		["1BC7:0021"] = {command="gsmctl -NgjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn -F"},--Telit
		["1BC7:1201"] = {command="gsmctl -VgjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn -F"},--Telit LTE
		["1BC7:0036"] = {command="gsmctl -gjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn -F"},--Telit LTE V2
		["12D1:1573"] = {command="gsmctl -UgjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn -F"}, --huawei
		["12D1:15C1"] = {command="gsmctl -UgjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn -F"}, --huawei TD LTE
		["12D1:15C3"] = {command="gsmctl -UgjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn -F"}, --huawei ME906s LTE
		["1199:68C0"] = {command="gsmctl -gjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn -F"}, --Siera
		["05C6:9215"] = {command="gsmctl -gjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn -F"}, --Quictel
		["05C6:9003"] = {command="gsmctl -gjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn -F"}, --Quictel UC20
		["2C7C:0125"] = {command="gsmctl -gjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn -F"} --Quictel EC25
	}

	if moduleVidPid then
		data = sys.exec(options[moduleVidPid]["command"])
...
function get_netw_type()
	...
	local options = {
		["1BC7:0021"] = {command="gsmctl -tn"},--Telit
		["1BC7:1201"] = {command="gsmctl -tn"},--Telit LTE
		["1BC7:0036"] = {command="gsmctl -tn"},--Telit LTE
		["12D1:1573"] = {command="gsmctl -tn"}, --huawei
		["12D1:15C1"] = {command="gsmctl -tn"}, --huawei TD LTE
		["12D1:15C3"] = {command="gsmctl -tn"}, --huawei TD LTE
		["1199:68C0"] = {command="gsmctl -tn"},
		["05C6:9215"] = {command="gsmctl -tn"},  --Quectel
		["05C6:9003"] = {command="gsmctl -tn"},  --Quectel UC20
		["2C7C:0125"] = {command="gsmctl -tn"}  --Quectel EC25
	}
...
local curlParams= "curl -i -m 5 --user \"user:" .. wimaxPass .. "\" --anyauth "
