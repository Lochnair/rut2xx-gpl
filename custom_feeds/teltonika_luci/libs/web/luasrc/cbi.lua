...
FileId:
$Id: cbi.lua 8158 2012-01-08 23:39:29Z jow $
...
-- Optimized? Not anymore...  Teltonika wuz here!
function Map.parse(self, readinput, ...)
	...
	if self.save then
		...
		if self:submitstate() and ((not self.proceed and self.flow.autoapply)) then
...
-- Add a hide dependencie to another section field
function AbstractValue.hide_deps(self, field, value)
	local deps
	if type(field) == "string" then
		deps = {}
	 	deps[field] = value
	...
	table.insert(self.hide_d, {deps=deps, add=""})
end
...
function Value.mathDepends(self, field, sum)
	local mathDeps
	if type(field) == "string" then
		mathDeps = {}
	 	mathDeps[field] = sum
	...
	table.insert(self.mathDeps, {mathDeps=mathDeps, add=""})
end
...
function Flag.parse(self, section)
	...
	if fexists then
		...
		if fvalue ~= self.default or (not self.optional and not self.rmempty) then
			...
			self:on_enable(section)
		else
			self:remove(section)
			self:on_disable(section)
		end
	else
		self:remove(section)
	end
...
function Flag.on_enable(self,section)

end

function Flag.on_disable(self,section)
...
Label = class(AbstractValue)

function Label.__init__(self, ...)
	AbstractValue.__init__(self, ...)
	self.template  = "cbi/label"
	self.keylist = {}
	self.vallist = {}
end

function Label.reset_values(self)
	self.keylist = {}
	self.vallist = {}
end

function Label.value(self, key, val)
	val = val or key
	table.insert(self.keylist, tostring(key))
	table.insert(self.vallist, tostring(val))
end
