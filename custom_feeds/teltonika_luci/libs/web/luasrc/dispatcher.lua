...
FileId:
$Id: dispatcher.lua 8018 2011-11-30 12:50:32Z jow $
...
branding = require "tlt_brand_lua"
_M.fs = fs
...
function username_validation(line)
    if line then
        line = line.gsub(line, '%W','')
    end
    return line
end
...
function authenticator.htmlauth(validator, accs, default)
	...
	local check_user = symbol_escape(luci.http.formvalue("username"))
        local user = username_validation(check_user)
	local pass = luci.http.formvalue("password")
	local ipaddr = symbol_escape(luci.http.getenv("REMOTE_ADDR"))
	...
	--User have changed, magically! Do not curse. Please >:3
	...
	if user == "admin" then
		...
		if user and validator(user, pass) then
			sys.exec("/usr/bin/logger \"WebUI password auth succeeded for 'admin' from ".. port .. " " .. ipaddr_eth .. " " .. ipaddr .."\"")
			t = {requests = "insert", table = "EVENTS", type="Web UI", text="Authentication was succesful from ".. port .. " " .. ipaddr_eth .. " " .. ipaddr ..""}
			eventlog:insert(t)
			--Delete first_login only if it is set and modem is detected
			sys.exec("[ \"$(uci -q get teltonika.sys.first_login)\" == \"1\" ] && (. /lib/teltonika-functions.sh; [ -z $(get_ext_vidpid_tlt) ] || (uci -q delete teltonika.sys.first_login; uci commit teltonika))")
			return user
		else
			sys.exec("/usr/bin/logger \"Bad WebUI password attempt for 'admin' from ".. port .. " " .. ipaddr_eth .. " " .. ipaddr .."\"")
		end
	elseif user then
		sys.exec("/usr/bin/logger \"WebUI Login attempt for nonexistent user from ".. port .. " " .. ipaddr_eth .. " " .. ipaddr .."\"")
		t = {requests = "insert", table = "EVENTS", type="Web UI", text="Authentication was not succesful for nonexistent user from ".. port .. " " .. ipaddr_eth .. " " .. ipaddr ..""}
		eventlog:insert(t)
	end
	if user then
		t = {requests = "insert", table = "EVENTS", type="Web UI", text="Authentication was not succesful from ".. port .. " " .. ipaddr_eth .. " " .. ipaddr ..""}
		eventlog:insert(t)
	end
	require("luci.i18n")
	require("luci.template")
...
function forcepassword(request)
	local pw_changed = uci:get("teltonika", "sys", "pass_changed")
	local exclude = { "password", "clock_status" }
	local flag = 0

	for key, value in ipairs(request) do
	...
	if (pw_changed == "0" and flag == 0) then
		http.redirect(build_url("admin/system/password"))
	end
end
...
function createtree()
	if not index then
		luci.sys.exec("rm -f /tmp/luci-indexcache 2> /dev/null")
		createindex()
	end
...
local function _cbi(self, ...)
	...
	local redirect
	local disclaimer_msg = false
	local email_to_sms = false
	local refreshpage
	local messages
	...
	for i, res in ipairs(maps) do
		...
		if res.disclaimer_msg then
			disclaimer_msg = disclaimer_msg or res.disclaimer_msg
		end

		if res.email_to_sms then
			email_to_sms = email_to_sms or res.email_to_sms
		end

		if res.refreshpage then
			refreshpage = refreshpage or res.refreshpage
		end
...
local function _arcombine(self, ...)
	...
	if #argv > 0 then
		if #argv > 1 then
			target = self.targets[3] or self.targets[2]
		else
...
function arcombine(trg1, trg2, trg3)
	return {type = "arcombine", env = getfenv(), target = _arcombine, targets = {trg1, trg2, trg3}}
end
