...
FileId:
$Id: sys.lua 8558 2012-04-15 14:30:54Z jow $
...
function mounts()
	...
	for line in ps do
		...
		if row[k[1]] then
			dir=row[k[1]]:sub(1,13)
			if dir ~= "/dev/mtdblock" then
				if row[k[6]]:find("/mnt/") then
...
function sambausers()
	local data = {}
	local k = {"username", "x", "id", "ids", "tu", "dir", "bin"}
	local ps = luci.util.execi("cat /etc/passwd")
	...
	for line in ps do
		...
		if tonumber(row[k[3]]) > 2000 and tonumber(row[k[3]]) < 3000  then			    
			table.insert(data, row)
		end
	end
...
function reboot()
	return os.execute("reboot -w >/dev/null 2>&1")
end
...
function net.devices(up)
	local devs = {}
	up = up or 0
	for k, v in ipairs(nixio.getifaddrs()) do
		if v.family == "packet"  then
			local u = v.flags
			if u.up == true or up == 0  then
				devs[#devs+1] = v.name
			end
		end
	end
