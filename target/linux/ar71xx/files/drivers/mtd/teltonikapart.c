...
#define TPLINK_UBOOT_LEN	0x20000
				   ...
				   struct mtd_part_parser_data *data)
{
	...
	kernel_offset = TPLINK_UBOOT_LEN + TPLINK_CONFIG_LEN + TPLINK_ART_LEN;

	header = tplink_read_header(master, kernel_offset);
	if (!header) {
	...
	squashfs_offset = kernel_offset + sizeof(struct tplink_fw_header) +
	...
	parts[0].name = "u-boot";
	parts[0].offset = 0;
	parts[0].size = TPLINK_UBOOT_LEN;
	parts[0].mask_flags = 0;
	
	parts[1].name = "config";
	parts[1].offset = TPLINK_UBOOT_LEN;
	parts[1].size = TPLINK_CONFIG_LEN;
	
	parts[2].name = "art";
	parts[2].offset = TPLINK_UBOOT_LEN + TPLINK_CONFIG_LEN;
	parts[2].size = TPLINK_ART_LEN;
