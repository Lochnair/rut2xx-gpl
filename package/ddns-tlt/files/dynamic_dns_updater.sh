...
Parameters:
 ...
 -i IP_INTERFACE     active connection interface
...
while getopts ":hv:n:S:i:V" OPT; do
	case "$OPT" in
		h)	usage; exit 0;;
		i)  IP_INTERFACE=$OPTARG;;
		v)	VERBOSE=$OPTARG;;
		n)	NETWORK=$OPTARG;;
		S)	SECTION_ID=$OPTARG;;
...
[ "$ip_source" = "network2" ] && {
	uci -q set ddns.$SECTION_ID.ip_source="interface"
	uci -q commit ddns
	ip_source="interface"
}
[ "$ip_source" = "interface" -a -n "$IP_INTERFACE" ] && {
	uci -q set ddns.$SECTION_ID.ip_interface="$IP_INTERFACE"
	uci -q commit ddns
...
while : ; do
	...
	if [ $CURR_TIME -ge $NEXT_TIME -o "$LOCAL_IP" != "$REGISTERED_IP" ]; then
		...
		if [ $ERR_LAST -eq 0 ]; then
			...
			echo "$timestamp1, $timestamp2" > "/tmp/ddns_status_$SECTION_ID"  # save last forced update time to config
