...
proto_3g_setup() {
	...
	# clean context to ensure if was use NCM protocol
	gsmctl -A "AT+CGDCONT=1"
	gsmctl -A "AT+CGDCONT=3"
	gsmctl -A "AT+CGDCONT=4"
	...
	case "$service" in
		*)
			...
			#Command do disconnect previos data connection if it was fail
