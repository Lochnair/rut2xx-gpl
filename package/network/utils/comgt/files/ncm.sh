...
proto_ncm_setup() {
	...
	local manufacturer initialize setmode connect devname devpath authenticate cid

	local device apn auth enabled username password auth_mode pincode delay mode pdptype ipv6 ifname method json_cid mtu
	json_get_vars device apn auth enabled username password auth_mode pincode delay mode pdptype ipv6 ifname method mtu
