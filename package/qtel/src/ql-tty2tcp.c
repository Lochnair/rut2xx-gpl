...
static const char *s_bind_device = NULL;

static unsigned s_uart_rate = 115200;
static unsigned s_uart_ctsrts = 0;

static unsigned uart_rates[] = {
    0, 9600, 19200, 38400, 57600, 115200, 230400, 460800, 921600
        , 1000000, 1152000, 1500000, 2000000, 2500000, 3000000, 3500000, 4000000
};

static speed_t uart_speeds[] = {
...
static void cfmakeserail(int ttyfd, int uart_rate, int ctsrts) {
    struct termios  ios;
    int status = TIOCM_DTR | TIOCM_RTS;
    speed_t uart_speed = B115200;
    unsigned i;

    for (i = 0; i < sizeof(uart_rates)/sizeof(uart_rates[0]); i++) {
        if (uart_rate == uart_rates[i]) {
            uart_speed = uart_speeds[i];
            break;
        }
    }
    ...
    if (ctsrts == 1)
        ios.c_cflag |= CRTSCTS; //enable the flow control on dev board
    else
        ios.c_cflag &= ~(CRTSCTS);//disable the flow control on dev board
    cfsetispeed(&ios, uart_speed);
    cfsetospeed(&ios, uart_speed);
    tcsetattr( ttyfd, TCSANOW, &ios );
    ioctl(ttyfd, TIOCMBIS, &status);
    //tcflush(ttyfd, TCIOFLUSH); //donot clear shahala msg report
}

static int open_serial(const char *tty_port, int uart_rate, int ctsrts) {
    int ttyfd;

    SYSCHECK(ttyfd = open (tty_port, O_RDWR | O_NDELAY));
    cfmakeserail(ttyfd, uart_rate, ctsrts);
...
unsigned int kfifo_used(struct __kfifo *fifo, void **pp_buf) {
    ...
    if (pp_buf)
        *pp_buf = len ? ((char *)fifo->data + off) : NULL;
...
static int connect_tcp_server(const char *tcp_host, int tcp_port)
{
    ...
    if (s_bind_device) {
        if (!isdigit(s_bind_device[0])) {
            dprintf("%s bind device %s\n", __func__, s_bind_device);
            SYSCHECK(setsockopt(sockfd, SOL_SOCKET, SO_BINDTODEVICE, (char *)s_bind_device, strlen(s_bind_device)));
        } else {
            struct sockaddr_in local_addr;
            bzero(&local_addr,sizeof(local_addr));
            local_addr.sin_family = AF_INET;
            local_addr.sin_addr.s_addr = inet_addr(s_bind_device);
            //ifconfig ethX:Y IP
            dprintf("%s bind IP %s\n", __func__, s_bind_device);
            SYSCHECK(bind(sockfd,(struct sockaddr*)&local_addr,sizeof(struct sockaddr)));
        }
    }
...
static void swap_fds(int swapfds[2], int fifo_sizes[2]) {
    struct __kfifo swapfifos[2];
    struct __kfifo *fifo;
    unsigned int len;
    void *pbuf =NULL;
    void *pwrap = malloc(max(fifo_sizes[0], fifo_sizes[1]));
    if (!pwrap) return;

    dprintf("%s fds={%d, %d} fifo_sizes={%d, %d}\n", __func__, swapfds[0], swapfds[1], fifo_sizes[0], fifo_sizes[1]);

    cfmakenoblock(swapfds[0]);
    cfmakenoblock(swapfds[1]);
    __kfifo_alloc(&swapfifos[0], fifo_sizes[0]);
    __kfifo_alloc(&swapfifos[1], fifo_sizes[1]);

    while (s_quit_flag == 0) {
        struct pollfd pollfds[] = {{swapfds[0], 0, 0}, {swapfds[1], 0, 0}};
        int ne, ret, nevents = sizeof(pollfds)/sizeof(pollfds[0]);

        pollfds[0].events |= kfifo_unused(&swapfifos[0], NULL) ? POLLIN : 0;
        pollfds[0].events |= kfifo_used(&swapfifos[1], NULL) ? POLLOUT : 0;
        pollfds[1].events |= kfifo_unused(&swapfifos[1], NULL) ? POLLIN : 0;
        pollfds[1].events |= kfifo_used(&swapfifos[0], NULL) ? POLLOUT : 0;
        ...
        if (ret < 0) {
            ...
            if (kfifo_used(&swapfifos[0], NULL))
                dprintf("%s can not send kfifo_used(0)=%d\n", __func__, kfifo_used(&swapfifos[0], NULL));
            if (kfifo_used(&swapfifos[1], NULL))
                dprintf("%s can not send kfifo_used(1)=%d\n", __func__, kfifo_used(&swapfifos[1], NULL));
            continue;
        }
        ...
        for (ne = 0; ne < nevents; ne++) {
            ...
            if (fd == swapfds[0] || fd == swapfds[1]) {
                if (revents & POLLIN) {
                    fifo = &swapfifos[(fd == swapfds[0] ? 0 : 1)];
                    len = kfifo_unused(fifo, &pbuf);

                    if (len) {
                        ssize_t nreads = read(fd, pbuf, len);
                        if (nreads <= 0) {
                        ...
                        if (nreads == len) {
                            len = kfifo_unused(fifo, &pbuf);
                            if (len) {
                                nreads = read(fd, pbuf, len);
                                if (nreads > 0)
                                    //dprintf("%s wrap read=%zd\n", __func__, nreads);
                                    __kfifo_in(fifo, nreads);
                            }
                        }
                    }
                ...
                if (revents & POLLOUT) {
                    fifo = &swapfifos[(fd == swapfds[0] ? 1 : 0)];
                    len = kfifo_used(fifo, &pbuf);

                    if (len) {
                        ssize_t nwrites;

                        if (0 < (fifo->in&fifo->mask) && (fifo->in&fifo->mask) < (fifo->out&fifo->mask)) {
                            memcpy(pwrap, pbuf, len);
                            memcpy(pwrap + len, fifo->data, (fifo->in&fifo->mask));
                            pbuf = pwrap;
                            len += (fifo->in&fifo->mask);
                            //dprintf("%s wrap write=%d\n", __func__, (fifo->in&fifo->mask));
                        }

                        nwrites = write(fd, pbuf, len);
                        if (nwrites <= 0) {
                        ...
                        if (kfifo_used(fifo, NULL) == 0)
                            __kfifo_reset(fifo); //to advoid wrap read
                    }
                }
            }
        }
...
__swap_fds_exit:
    free(pwrap);
    __kfifo_free(&swapfifos[0]);
    __kfifo_free(&swapfifos[1]);
    return;
}
    ...
    while (s_quit_flag == 0) {
        ...
        for (i = 0; i < 2; i++) {
            if (sources[i][0] == 's') {
                ...
                if (sources[i][2] == '/')
                    swapfds[i] = open_serial(&sources[i][2], s_uart_rate, s_uart_ctsrts);
        ...
        if (swapfds[0] > 0 && swapfds[1] > 0)
            swap_fds(swapfds, fifo_sizes);

        if (swapfds[0] > 0) {
			if (is_tcp_socket[0])
				shutdown(swapfds[0], SHUT_RDWR);
			close(swapfds[0]);
		}

        if (swapfds[1] > 0) {
			if (is_tcp_socket[1])
				shutdown(swapfds[1], SHUT_RDWR);
			close(swapfds[1]);
		}
...
//c:fd                                    open fd, like 0 ~ stdin, 1 ~ stdout
    ...
    optind = 1;
    while ( -1 != (opt = getopt(argc, argv, "s:c:d:p:i:r:f:"))) {
        switch (opt) {
            case 's':
            ...
            break;
            case 'i':
                s_bind_device = strdup(optarg);
            break;
            case 'r':
                s_uart_rate = atoi(optarg);
            break;
            case 'f':
                s_uart_ctsrts = !!atoi(optarg);
            break;
            default:
            break;
        }
