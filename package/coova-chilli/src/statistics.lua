...
function update()
	...
	local enviroment = {
		...
		period = "DATA_INTERVAL"
	}
	local results = get_env(enviroment)
	local db = open_DB(dbPath, dbName)
	
	if results and db then
		query = string.format("UPDATE statistics SET  end_time=:end_time, duration=:duration, input=:input, output=:output WHERE mac='%s' AND session=1", results.mac)
		local stmt = db:prepare(query)
		...
		if results.localusers then
			...
			if (user_data[1].download and tonumber(user_data[1].download) >= tonumber(results.max_input)) or
					(user_data[1].upload and tonumber(user_data[1].upload) >= tonumber(results.max_output)) then
