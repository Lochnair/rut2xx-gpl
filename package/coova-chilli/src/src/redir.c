...
// Funkcija skirta url parametru keitimui
char *url_parameters(char *option) {
	struct uci_context *uci;
	uci = uci_init();

	char *url_parameter = NULL;
	char *new_str;

	url_parameter = ucix_get_option(uci, "coovachilli", "url", option);
	if (url_parameter == NULL){
		if((new_str = malloc(strlen(option)+2)) != NULL){
			sprintf(new_str,"%s", option);
		} else {
			log_err(errno, "url_option malloc failed!\n");
		}
	} else if((new_str = malloc(strlen(url_parameter)+2)) != NULL){
		sprintf(new_str,"%s", url_parameter);
	} else {
		log_err(errno, "url_parameter malloc failed!\n");
	}
	uci_cleanup(uci);
	free(url_parameter);
  return new_str;
...
// Funkcija skirta custom url parametru keitimui
char *custom_url_parameters(char *option) {
	struct uci_context *uci;
	uci = uci_init();

	char *custom_option = NULL;

	custom_option = ucix_get_option(uci, "coovachilli", "url", option);
	uci_cleanup(uci);
...
static void bstring_buildurl(bstring str, struct redir_conn_t *conn,
  ...
  bstring bt2 = bfromcstr("");
	...
	bassignformat(str, "%s%sres=%s%s%s=%s%s%s=%d",
		...
		url_parameters("uamip"), inet_ntoa(redir->addr), amp,
		url_parameters("uamport"), redir->port);

	if (custom_url_parameters("custom_name_nr1") != NULL && custom_url_parameters("custom_value_nr1") != NULL){
		bcatcstr(str, amp);
    bassignformat(bt, "%s=%s", custom_url_parameters("custom_name_nr1"), custom_url_parameters("custom_value_nr1"));
    bconcat(str, bt);
  } else if (custom_url_parameters("custom_name_nr2") != NULL && custom_url_parameters("custom_value_nr2") != NULL){
		bcatcstr(str, amp);
    bassignformat(bt, "%s=%s", custom_url_parameters("custom_name_nr2"), custom_url_parameters("custom_value_nr2"));
    bconcat(str, bt);
  }

  if (hexchal) {
    bcatcstr(str, amp);
    bassignformat(bt, "%s=%s", url_parameters("challenge"), hexchal);
    bconcat(str, bt);
    bassigncstr(bt,"");
  }
  ...
  bcatcstr(str, amp);
  bassignformat(bt, "%s=", url_parameters("called"));
  bconcat(str, bt);
  if (_options.nasmac)
  ...
  if (hismac) {
    bcatcstr(str, amp);
    bassignformat(bt, "%s=", url_parameters("mac"));
    bconcat(str, bt);
    bassignformat(bt, "%.2X-%.2X-%.2X-%.2X-%.2X-%.2X",
		  hismac[0], hismac[1],
		  hismac[2], hismac[3],
  ...
  if (hisip) {
    bcatcstr(str, amp);
    bassignformat(bt, "%s=%s", url_parameters("ip"), inet_ntoa(*hisip));
    bconcat(str, bt);
    bassigncstr(bt,"");
  }
  ...
  if (_options.radiusnasid) {
    bcatcstr(str, amp);
    bassignformat(bt, "%s=", url_parameters("nasid"));
    bconcat(str, bt);
  ...
  if (conn->s_state.sessionid[0]) {
    bcatcstr(str, amp);
    bassignformat(bt, "%s=", url_parameters("sessionid"));
    bconcat(str, bt);
  ...
  if (userurl) {
    bcatcstr(str, amp);
    bassignformat(bt, "%s=", url_parameters("userurl"));
    bconcat(str, bt);
...
void redir_wispr1_reply (struct redir_t *redir, struct redir_conn_t *conn,
  ...
  case REDIR_NOTYET:
    ...
    bassignformat(bt, "<LoginURL>%s%sres=wispr&amp;%s=%s&amp;%s=%d&amp;%s=%s</LoginURL>\r\n",
		  ...
		  url_parameters("uamip"), inet_ntoa(redir->addr), url_parameters("uamport"), redir->port, url_parameters("challenge"), hexchal);
    bconcat(b, bt);
...
int redir_reply(struct redir_t *redir, struct redir_socket_t *sock,
  ...
  case REDIR_CHALLENGE:
    ...
    break;
  case REDIR_BLOCKED:
	resp = "failed&reason=blocked";
	break;
  case REDIR_BLOCKED_TIMEOUT:
	resp = "failed&reason=timelimit";
	break;
  default:
  ...
  if (resp) {
    ...
    if ( res == REDIR_SUCCESS && _options.successurl ){
		bt = bfromcstralloc(128, "");
		bassignformat(bt, "%s", _options.successurl);
      	bconcat(buffer, bt );
		bdestroy(bt);
    }
    else if (url) {
...
static int redir_getreq(struct redir_t *redir, struct redir_socket_t *sock,
  ...
  case REDIR_LOGIN:
	...
	return -1;
      ...
      if (!redir_getparam(redir, httpreq->qs, url_parameters("userurl"), bt)) {
				log_dbg("-->> userurl: %s",url_parameters("userurl"));
	bstring bt2 = bfromcstr("");
...
int check_user(const char * user, struct session_params *params, char *download, char *upload){
	FILE *pCMD;
	char result[256]; char script[1024];

	if (!params->start_point || !_options.dhcpif || !user){
 		log_dbg("Parameters: user %s, ifname %s", user ? "found" : "not found", _options.dhcpif ? "found" : "not found");
 		log_dbg("Parameters: start_point %s", params->start_point ? "found" : "not found");
		return 0;
	}

	if (params->maxinputoctets && params->maxinputoctets){
		log_dbg("/usr/sbin/statistics check %s %s %d %d 2>&1", user, _options.dhcpif, params->data_interval , params->start_point)
		sprintf(script, "/usr/sbin/statistics check %s %s %d %d 2>&1", user, _options.dhcpif, params->data_interval, params->start_point);
	}
	else if (_options.defmaxinputoctets && _options.defmaxoutputoctets){
		log_dbg("/usr/sbin/statistics check %s %s %d %d 2>&1", user, _options.dhcpif, _options.defdatainterval , _options.defstartpoint)
		sprintf(script, "/usr/sbin/statistics check %s %s %d %d 2>&1", user, _options.dhcpif, _options.defdatainterval , _options.defstartpoint);
	}
	else
		return 0;

	if (!(pCMD = popen(script, "r"))) {
		return 0;
	} else {
		fgets(result, 256, pCMD);
		if (sizeof(result) > 4) {
			char *pl=result,
			     *p_download = download,
			     *p_upload = upload;

			/* download until the first ':' */
			while (*pl && *pl != ':' && *pl != '\n')	*p_download++ = *pl++;

			/* skip over ':' otherwise error */
			if (*pl == ':') pl++;
			else {
				log_warn(0, "not a valid line: %s", result);
			}
...
int check_mac(uint8_t *mac, char *download, char *upload, char *time){
	FILE *pCMD;
	char result[256]; char script[1024];


	if (mac){
		log_dbg("/usr/sbin/statistics check_mac_all %.2X-%.2X-%.2X-%.2X-%.2X-%.2X %s %d %d 2>&1", mac[0], mac[1], mac[2],
			mac[3], mac[4], mac[5], _options.dhcpif, _options.defdatainterval , _options.defstartpoint)
		sprintf(script, "/usr/sbin/statistics check_mac_all %.2X-%.2X-%.2X-%.2X-%.2X-%.2X %s %d %d 2>&1", mac[0], mac[1],
			mac[2], mac[3], mac[4], mac[5], _options.dhcpif, _options.defdatainterval , _options.defstartpoint);
	}
	else
		return 0;
...
int is_local_user(struct redir_t *redir, struct redir_conn_t *conn) {
  uint8_t user_password[RADIUS_PWSIZE+1];
  uint8_t chap_challenge[REDIR_MD5LEN];
  char u[256]; char p[256]; char ses_timeout[256]; char idle_timeout[256]; char dw_limit[256];
  char upl_limit[256]; char dw_bandwidth[256]; char upl_bandwidth[256]; char start_point[256]; char data_interval[256];
  char download[256]; char upload[256];
  ...
  while ((len = getline(&line, &sz, f)) > 0) {
    if (len > 3 && len < sizeof(u) && line[0] != '#') {
	   ...
	   *pu=u,     				/* pointer to username     */
  	   ...
  	   *p_ses_timeout=ses_timeout,		/* pointer to session timeout     */
	   *p_idle_timeout=idle_timeout,	/* pointer to idle timeout     */
	   *p_dw_limit=dw_limit,		/* pointer to download limit     */
	   *p_upl_limit=upl_limit,		/* pointer to upload limit     */
	   *p_dw_bandwidth=dw_bandwidth,	/* pointer to period     */
	   *p_upl_bandwidth=upl_bandwidth,	/* pointer to start     */
	   *p_start_point = start_point,	/*pointer to start point*/
	   *p_data_interval = data_interval;	/*pointer to data interval*/
      ...
      *pp = 0;
		...
		if (conn->authdata.type == REDIR_AUTH_PAP) {
			...
			if (!memcmp(user_password, tmp,  REDIR_MD5LEN)){
				...
				log_dbg("bad password %s for %s", p, u);
			}
		}
		...
		if (match == 1){
			...
			if (*pl == ':') pl++;
				...
				log_warn(0, "not a valid session timeout line: %s", line);
			}

			/* session timeout until the next ':' */
			while (*pl && *pl != ':' && *pl != '\n') *p_ses_timeout++ = *pl++;

			/* skip over ':' otherwise error */
			if (*pl == ':') pl++;
			else {
				log_warn(0, "not a valid idle timeout line: %s", line);
			}

			/* idle timeout until the next ':' */
			while (*pl && *pl != ':' && *pl != '\n') *p_idle_timeout++ = *pl++;

			/* skip over ':' otherwise error */
			if (*pl == ':') pl++;
			else {
				log_warn(0, "not a valid download limit line: %s", line);
			}

			/* download limit until the next ':' */
			while (*pl && *pl != ':' && *pl != '\n') *p_dw_limit++ = *pl++;

			/* skip over ':' otherwise error */
			if (*pl == ':') pl++;
			else {
				log_warn(0, "not a valid upload limit line: %s", line);
			}

			/* upload limit until the next ':' */
			while (*pl && *pl != ':' && *pl != '\n') *p_upl_limit++ = *pl++;

			/* skip over ':' otherwise error */
			if (*pl == ':') pl++;
			else {
				log_warn(0, "not a valid period line: %s", line);
			}
			...
			if (*pl == ':') pl++;
				...
				log_warn(0, "not a valid start line: %s", line);
			}
			...
			if (*pl == ':') pl++;
				...
				log_warn(0, "not a valid start point line: %s", line);
			}
			...
			if (*pl == ':') pl++;
				...
				log_warn(0, "not a valid data interval line: %s", line);
			}
			...
			*p_data_interval=0;
			...
			if (atoi(ses_timeout) != 0)
				conn->s_params.sessiontimeout = atoi(ses_timeout);
			if (atoi(idle_timeout) != 0)
				conn->s_params.idletimeout = atoi(idle_timeout);
			if (atoi(dw_limit) != 0)
				conn->s_params.maxinputoctets = atoi(dw_limit);
			if (atoi(upl_limit) != 0)
				conn->s_params.maxoutputoctets = atoi(upl_limit);
			if (atoi(dw_bandwidth) != 0)
				conn->s_params.bandwidthmaxdown = atoi(dw_bandwidth);
			if (atoi(upl_bandwidth) != 0)
				conn->s_params.bandwidthmaxup = atoi(upl_bandwidth);
			if (atoi(start_point) != 0)
				conn->s_params.start_point = atoi(start_point);
			if (atoi(data_interval) != 0)
				conn->s_params.data_interval = atoi(data_interval);

			if (check_user(conn->s_state.redir.username, &conn->s_params, download, upload)){
				if (atoi(download) > conn->s_params.maxinputoctets)
					match = 2;
				else if (atoi(upload) > conn->s_params.maxoutputoctets)
					match = 2;
			}
			else {
				log_dbg("Can't get user %s data.", u);
			}
		}
		break;
       }
...
int is_mac(struct redir_t *redir, struct redir_conn_t *conn) {
	log_dbg("mac auth mode");
  uint8_t user_password[RADIUS_PWSIZE+1];
  uint8_t chap_challenge[REDIR_MD5LEN];
  char p[256];
  char download[256]; char upload[256]; char time[256];
  size_t sz=1024;
  ssize_t len;
  int match=0;
  char *line=0;
  ...
  while ((len = getline(&line, &sz, f)) > 0) {
	...
	if (conn->authdata.type == REDIR_AUTH_PAP) {
		...
		if (!memcmp(user_password, tmp,  REDIR_MD5LEN)){
			...
			if (check_mac(conn->hismac, download, upload, time)){
				if (_options.defmaxinputoctets > 0 && atoi(download) > _options.defmaxinputoctets)
					match = 2;
				else if (_options.defmaxoutputoctets > 0 && atoi(upload) > _options.defmaxoutputoctets)
					match = 2;
				else if (_options.defsessiontimeout > 0 && atoi(time) > _options.defsessiontimeout)
					match = 3;
			}
			else
			{
				log_dbg("Can't get data.");
				match = 1;
			}
			break;
		}
		else {
			log_dbg("bad password %s", p);
		}
	}
    }
  }

  log_dbg("pass %s ", match ? "correct" : "incorrect");
...
int is_sms_one_time_password(struct redir_t *redir, struct redir_conn_t *conn) {
  uint8_t user_password[RADIUS_PWSIZE+1];
  uint8_t chap_challenge[REDIR_MD5LEN];
  char u[256];
  char download[256]; char upload[256]; char time[256];
  ...
  log_dbg("SMS otp mode. Checking %s for user %s", _options.localusers, conn->s_state.redir.username);
  ...
  while ((len = getline(&line, &sz, f)) > 0) {
    if (len > 3 && len < sizeof(u) && line[0] != '#') {
      ...
      *pu = 0;
		...
		if (check_mac(conn->hismac, download, upload, time)){
			log_dbg("CHECK_MAC_DOWNLOAD: %s >>>>>>> %d", download, _options.defmaxinputoctets);
			log_dbg("CHECK_MAC_UPLOAD: %s >>>>>>> %d", upload, _options.defmaxoutputoctets);
  ...
  case REDIR_LOGIN: {
	...
	mainclock_now()) {
	 ...
	 if (_options.localusers){
		...
		if (_options.smsotpmode){
			log_dbg("REDIR SMSOPTMODE!!!!!!!!");
			match = is_sms_one_time_password(redir, &conn);
		}
		else if (_options.macmode){
			log_dbg("REDIR IS_MAC!!!!!!!!", match);
			match = is_mac(redir, &conn);
		}
		else{
			log_dbg("REDIR IS LOCAL USER!!!!!!!!: %d", match);
			match = is_local_user(redir, &conn);
		}
	 }

    if (match) {
		log_dbg("REDIR MATCH!!!!!!!!: %d", match);
	    if (match == 2)
		conn.response = REDIR_BLOCKED;
	    else if (match == 3)
		conn.response = REDIR_BLOCKED_TIMEOUT;
	    else {
    ...
    char download[256]; char upload[256];
    if (!_options.localusers && check_user(conn.s_state.redir.username, &conn.s_params, download, upload)){

		log_dbg("Tikrina download su leistinu!!!!!!!!: %s == %d", download, conn.s_params.maxinputoctets);
		log_dbg("Tikrina upload su leistinu!!!!!!!!: %s == %d", upload, conn.s_params.maxoutputoctets);
	...
	log_dbg("Tikrina response su leistinu!!!!!!!!: %d == %d", conn.response, REDIR_SUCCESS);
	...
	conn.s_params.flags |= IS_UAM_REAUTH;
      ...
      if (_options.redirect){
        log(LOG_NOTICE, "Redirecting to user url: %s", conn.s_state.redir.userurl);
        ...
        (char *)conn.s_state.redir.userurl,
        conn.hismac, &conn.hisip, httpreq.qs);
      }
      else {
				log(LOG_NOTICE, "Redirecting to success page");
        redir_reply(redir, &socket, &conn, REDIR_SUCCESS, NULL,
  		  conn.s_params.sessiontimeout, NULL,
  		  conn.s_state.redir.username,
  		  conn.s_state.redir.userurl, conn.reply,
