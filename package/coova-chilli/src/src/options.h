...
struct options_t {
  ...
  char * hotspotid;                /* Script to run after session/connection-down */
  char * macup;
  char * macdown;
  int txqlen;
...
#define EXTADMVSA_ATTR_CNT 4
  ...
  uint64_t defmaxinputoctets;
  uint64_t defmaxoutputoctets;
  uint32_t defdatainterval;
  uint32_t defstartpoint;
