...
leaky_bucket(struct app_conn_t *conn,
  ...
  if (conn->s_params.bandwidthmaxup) {
	...
	conn->s_state.bucketupsize) {
      if (_options.debug)
	log_dbg("Leaky bucket dropping upload overflow ");
      result = -1;
    }
    else {
  ...
  if (conn->s_params.bandwidthmaxdown) {
	...
	conn->s_state.bucketdownsize) {
      if (_options.debug)
				log_dbg("Leaky bucket dropping download overflow to ");
      result = -1;
    }
    else {
  ...
  if (_options.debug &&
    ...
    if (_options.debug)
      log_dbg("%s(%d): Leaky bucket: bucketup: %lld/%lld, " __FUNCTION__, __LINE__,
...
int runscript_test(struct app_conn_t *appconn, char* script) {
  int status;
  ...
  set_env("INPUT_OCTETS_SESSION", VAL_ULONG64, &appconn->s_state.input_octets_session, 0);
  set_env("OUTPUT_OCTETS_SESSION", VAL_ULONG64, &appconn->s_state.output_octets_session, 0);

  if (_options.localusers) {
	  log_err(errno, "start_point %d", appconn->s_params.start_point);
	  log_err(errno, "data_interval %d", appconn->s_params.data_interval);
	  set_env("LOCALUSERS", VAL_STRING, _options.localusers, 0);
	  set_env("START_POINT", VAL_ULONG, &appconn->s_params.start_point, 0);
	  set_env("DATA_INTERVAL", VAL_ULONG, &appconn->s_params.data_interval, 0);
  }
...
int runscript(struct app_conn_t *appconn, char* script,
...
#endif
  set_env("HOTSPOTID", VAL_STRING, _options.hotspotid, 0);
  set_env("DHCPIF", VAL_STRING, _options.dhcpif, 0);
  ...
  if (_options.smsotpmode)
      set_env("SMSOTPMODE", VAL_ULONG, &_options.smsotpmode, 0);
...
static int checkconn() {
  ...
  for (conn = firstusedconn; conn; conn=conn->next) {
	...
	continue;
      ...
      if ( skip_update >= 10 ){
		 if (conn->s_state.authenticated == 1) {
			runscript_test(conn, "/etc/chilli/update.sh");
		 }
		    ...
		    struct app_conn_t *conn, uint8_t status_type)
{
  ...
  case RADIUS_STATUS_TYPE_ACCOUNTING_ON:
    ...
    conn->s_state.last_sent_time = mainclock.tv_sec;
    switch(type) {
#ifdef ENABLE_GARDENACCOUNTING
    case ACCT_GARDEN:
	...
	conn->s_state.input_octets_session = 0;
	conn->s_state.output_octets_session = 0;

	if (((_options.smsotpmode || _options.macmode) && check_mac(conn->hismac, download, upload, time)) ||
		(!_options.smsotpmode && !_options.macmode  && check_user(conn->s_state.redir.username, &conn->s_params, download, upload))){

		if (conn->s_params.maxinputoctets && conn->s_params.maxinputoctets > 0)
			conn->s_state.input_octets = (int64_t)atoi(download);

		if (conn->s_params.maxoutputoctets && conn->s_params.maxoutputoctets > 0)
			conn->s_state.output_octets = (int64_t)atoi(upload);

		if (conn->s_params.sessiontimeout && conn->s_params.sessiontimeout > 0){
			if ((int64_t)atoi(time) > conn->s_params.sessiontimeout)
				conn->s_params.sessiontimeout = 0;
			else
				conn->s_params.sessiontimeout = conn->s_params.sessiontimeout - (int64_t)atoi(time);
		}
	}

 	log_dbg("input_octets_session: %lld", conn->s_state.input_octets);
 	log_dbg("output_octets_session: %lld", conn->s_state.output_octets);
  	log_dbg("Start point: %lld", conn->s_params.start_point);
  	log_dbg("Data interval: %lld", conn->s_params.data_interval);
	log_dbg("s_state.start_time: %d", conn->s_state.start_time);
	log_dbg("s_state.start_time: %d", conn->s_state.start_time + 1);
	log_dbg("time: %s", time);
	log_dbg("appconn->s_params.sessiontimeout: d", conn->s_params.sessiontimeout);
      break;
    }
    break;
...
chilli_learn_location(uint8_t *loc, int loclen,
  ...
  if (appconn->s_state.authenticated == 1 &&
    ...
    appconn->s_state.last_sent_time = mainclock.tv_sec;
...
void session_param_defaults(struct session_params *params) {
  ...
  if (_options.defmaxinputoctets && !params->maxinputoctets)
    params->maxinputoctets = _options.defmaxinputoctets;

  if (_options.defmaxoutputoctets && !params->maxoutputoctets)
    params->maxoutputoctets = _options.defmaxoutputoctets;

  if (_options.defdatainterval && !params->data_interval)
    params->data_interval = _options.defdatainterval;

  if (_options.defstartpoint && !params->start_point)
    params->start_point = _options.defstartpoint;
}
...
config_radius_session(struct session_params *params,
  ...
  if (!radius_getattr(pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC,
		      RADIUS_VENDOR_CHILLISPOT,
		      RADIUS_ATTR_CHILLISPOT_START_POINT, 0))
    params->start_point = ntohl(attr->v.i);
  else if (!reconfig)
  ...
  if (!radius_getattr(pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC,
		      RADIUS_VENDOR_CHILLISPOT,
		      RADIUS_ATTR_CHILLISPOT_DATA_INTERVAL, 0))
    params->data_interval = ntohl(attr->v.i);
  else if (!reconfig)
    params->data_interval = 0;
...
clear_appconn(struct app_conn_t *appconn) {
  appconn->s_params.start_point =
  appconn->s_params.data_interval =
  appconn->s_params.bandwidthmaxup =
    appconn->s_params.bandwidthmaxdown =
    appconn->s_params.maxinputoctets =
    appconn->s_params.maxoutputoctets =
...
int chilli_acct_fromsub(struct app_conn_t *appconn,
  ...
  if (appconn->s_state.authenticated == 1) {
	...
	appconn->s_state.input_octets +=len;
	 admin_session.s_state.input_octets_session+=len;
	if (admin_session.s_state.authenticated) {
	...
	appconn->s_state.output_octets_session +=len;
	if (admin_session.s_state.authenticated) {
	  admin_session.s_state.output_packets++;
	  admin_session.s_state.output_octets+=len;
	  admin_session.s_state.output_octets_session+=len;
	}
      }
    }
...
int chilli_acct_tosub(struct app_conn_t *appconn,
  ...
  if (appconn->s_state.authenticated == 1) {
	...
	appconn->s_state.input_packets++;
	appconn->s_state.input_octets += len;
	appconn->s_state.input_octets_session += len;
	if (admin_session.s_state.authenticated) {
	  admin_session.s_state.input_packets++;
	  admin_session.s_state.input_octets+=len;
	  admin_session.s_state.input_octets_session+=len;
