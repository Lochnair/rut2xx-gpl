...
function get_names()
	--Default names
	local names = {
		button = "button", send = "send", TelNum = "TelNum", res = "res", reason = "reason", sms = "sms",
		UserName = "UserName", reply = "reply", Password = "Password", redirurl = "redirurl", agree_tos = "agree_tos"
	}
...
function get_ifname(id)
	local ubus = bus.connect()
	local info = ubus:call("network.wireless", "status", { })

	if info.radio0.interfaces then
		local i, iface
		for i, iface in ipairs(info.radio0.interfaces) do
			if iface.section then
				local hotspot_id = uci:get("wireless", iface.section, "hotspotid") or ""
				if hotspot_id == id then
					debug("IFNAME: ".. iface.ifname and iface.ifname or "no")
					return iface.ifname and iface.ifname
				end
			end
		end
	end
...
function append_smsusers_file(file, phone, code)
	if fs.access(file, "r") then
		local buffer = ""
		for line in io.lines(file) do
			if not find(line, phone) then
				buffer = buffer .. line .. "\n"
			end
		end

		buffer = buffer .. code .. ":" .. phone .. "\n"
		fs.writefile( file, buffer )
	end
end

function send_sms(number, message)
	local msg = number .. " " .. message
	local res = getParam(string.format("/usr/sbin/gsmctl -Ss %s", escape(msg)))
	return (res == "OK" or find(res, "+CMGS:"))
end
